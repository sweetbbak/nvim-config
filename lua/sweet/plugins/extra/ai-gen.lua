    ...
    config = function()
        ...
        gen.setup({
            ...
            init = function(options)
                pcall(io.popen, "pgrep ollama||ollama serve > /dev/null 2>&1 &")
            end,
            -- Function to initialize Ollama
            -- command = "curl --silent --no-buffer -X POST http://localhost:11434/api/generate -d $body",
            -- command = "echo $model-$prompt-$body > ~/output && curl http://localhost:11434/api/generate -d '{model: $model, prompt: $prompt}'",
